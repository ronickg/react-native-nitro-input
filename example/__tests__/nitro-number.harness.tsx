/**
 * On-device checks of `NitroNumber`: what an app can observe through the
...
describe('NitroNumber', () => {
  it('mounts, hands out its native ref and lays out to its content', async () => {
    ...
    await render(<NitroNumber ref={ref} value={1234.5} fractionDigits={2} style={content} onLayout={onLayout} />)
  ...
  it('widens as soon as a digit appears and narrows only once the roll has finished', async () => {
    const ref = createRef<NitroNumberHandle>()
    const { state, onLayout } = layoutOf()
    const { rerender } = await render(<NitroNumber ref={ref} value={999} duration={200} style={content} onLayout={onLayout} />)
    await waitFor(() => expect(state.current?.width ?? 0).toBeGreaterThan(0))
    const threeDigits = state.current!.width

    await rerender(<NitroNumber ref={ref} value={1000000} duration={200} style={content} onLayout={onLayout} />)
    ...
    await rerender(<NitroNumber ref={ref} value={5} duration={200} style={content} onLayout={onLayout} />)
  ...
  it('plays the numeric transition and settles on the value, columns included', async () => {
    ...
    const { rerender } = await render(<NitroNumber ref={ref} value={99} {...props} />)
    await waitFor(() => expect(state.current?.width ?? 0).toBeGreaterThan(0))
    const twoDigits = state.current!.width

    // A change that grows a column: the box widens as the new glyph arrives.
    await rerender(<NitroNumber ref={ref} value={100} {...props} />)
    ...
    for (const v of [105, 118, 123, 131]) {
      await rerender(<NitroNumber ref={ref} value={v} {...props} />)
      await sleep(40)
    }
    ...
    await rerender(<NitroNumber ref={ref} value={7} {...props} />)
    ...
    await rerender(<NitroNumber ref={ref} value={43} transition="roll" duration={150} style={content} onLayout={onLayout} />)
  ...
  it('plays the scramble and settles on the value', async () => {
    for (const transition of ['scramble'] as const) {
      ...
      const { rerender, unmount } = await render(<NitroNumber ref={ref} value={95} {...props} />)
      await waitFor(() => expect(state.current?.width ?? 0).toBeGreaterThan(0))
      const twoDigits = state.current!.width

      // Up through the wrap and into a new column, then back down.
      await rerender(<NitroNumber ref={ref} value={108} {...props} />)
  ...
  it('flashes and pops on a change without disturbing the value or the layout', async () => {
    ...
    const { rerender } = await render(<NitroNumber ref={ref} value={500} {...props} />)
    await waitFor(() => expect(state.current?.width ?? 0).toBeGreaterThan(0))
    const width = state.current!.width
    await rerender(<NitroNumber ref={ref} value={501} {...props} />)
    await waitFor(() => expect(ref.current?.getValue()).toBe(501))
    await rerender(<NitroNumber ref={ref} value={499} {...props} />)
  ...
  it('takes imperative values through the handle', async () => {
    const ref = createRef<NitroNumberHandle>()
    await render(<NitroNumber ref={ref} value={1} fractionDigits={1} duration={100} />)
    await waitFor(() => expect(ref.current?.native).not.toBeNull())
  ...
  it('plays a count reveal through its milestones and reports the landing', async () => {
    ...
    const { rerender } = await render(<NitroNumber {...props} reveal={false} />)
    // Held at the opening frame: nothing has fired.
    await sleep(300)
    expect(milestones).toEqual([])

    await rerender(<NitroNumber {...props} reveal />)
  ...
  it('sizes with the font and with the affixes, and keeps its size while loading', async () => {
    const { state, onLayout } = layoutOf()
    const { rerender } = await render(<NitroNumber value={1234567} fontSize={20} style={content} onLayout={onLayout} />)
    await waitFor(() => expect(state.current?.width ?? 0).toBeGreaterThan(0))
    const small = { ...state.current! }

    await rerender(<NitroNumber value={1234567} fontSize={40} style={content} onLayout={onLayout} />)
    ...
    await rerender(<NitroNumber value={1234567} fontSize={40} groupingSeparator="," prefix="$" suffix=" USD" style={content} onLayout={onLayout} />)
    ...
    await rerender(<NitroNumber value={1234567} fontSize={40} groupingSeparator="," prefix="$" suffix=" USD" loading style={content} onLayout={onLayout} />)
    await sleep(400)
  ...
  it('mirrors under a right-to-left layout without changing its size', async () => {
    ...
    await render(
      <View>
        <NitroNumber value={1234.5} fractionDigits={2} prefix="$" suffix=" USD" groupingSeparator="," style={content} onLayout={ltr.onLayout} />
        <NitroNumber
          value={1234.5}
          fractionDigits={2}
          prefix="$"
  ...
  it('keeps a fixed box while shrinking the figure to fit', async () => {
    ...
    const { rerender } = await render(
      <NitroNumber value={1} adjustsFontSizeToFit minimumFontScale={0.3} style={{ width: 80 }} onLayout={onLayout} />,
    )
    await waitFor(() => expect(state.current?.width ?? 0).toBeGreaterThan(0))
    expect(state.current!.width).toBeCloseTo(80, 0)
    await rerender(
      <NitroNumber value={123456789} adjustsFontSizeToFit minimumFontScale={0.3} style={{ width: 80 }} onLayout={onLayout} />,
    )
    await sleep(700)
    expect(state.current!.width).toBeCloseTo(80, 0)
  })
  ...
  it('drives thirty figures at once', async () => {
    const refs = Array.from({ length: 30 }, () => createRef<NitroNumberHandle>())
    const tree = (scale: number) => (
      <View>
        {refs.map((ref, i) => (
          <NitroNumber key={i} ref={ref} value={i * scale} fractionDigits={2} groupingSeparator="," prefix="$" duration={150} />
        ))}
      </View>
    )
...
describe('NitroNumber lifetime', () => {
  ...
  it('frees its views on unmount and mounts working copies after the churn', async () => {
    const copies = 24
    const cycles = 30
    const refs = Array.from({ length: copies }, () => createRef<NitroNumberHandle>())
    const tree = (base: number) => (
      <View>
        {refs.map((ref, i) => (
          <NitroNumber key={i} ref={ref} value={base + i} duration={100} style={content} />
        ))}
      </View>
    )
