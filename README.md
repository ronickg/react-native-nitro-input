...
- [`packages/react-native-nitro-rolling-number`](packages/react-native-nitro-rolling-number) – the library: the shared C++ engine (`cpp/`), the Swift and Kotlin views, the Nitro spec and the JS wrapper. Its [README](packages/react-native-nitro-rolling-number/README.md) is the API reference.
...
- [`docs/`](docs) – the Docusaurus site. Its live demos run the very same `RollingEngine.cpp`, compiled to WebAssembly.
...
bun --cwd packages/react-native-nitro-rolling-number run test:cpp   # engine tests
