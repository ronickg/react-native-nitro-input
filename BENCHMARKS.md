...
- **Memory.** Two ways to leak: 24 copies (20 fields) mounted and unmounted
  ...
  reads memory after a forced collection before and with them, and divides
  the difference by the count: what one copy costs. Instruments' Leaks template was meant as the iOS
...
count. A mounted Nitro number is about 60 KB on all three phones (54 KB of
malloc and 9 KB of Java heap on the A22, 57 KB of malloc on the iPhones);
plain `Text` is 81 KB on the A22 and 122 KB on the iPhones, where its layer
backing store is what costs. A NitroInput field is 93 KB on the A22 and
135–140 KB on the iPhones, a `TextInput` 102 and 113–136; a MorphInput is
79–96 KB on the iPhones and 242 KB on the A22, where its glyph engine's
per-field allocations stand out and are worth a look. These are the figures
behind the `memorySize` the hybrids report to Nitro (64 KB for a number,
128 KB for a field): what the JS garbage collector is told a handle stands
for. The tables cover the native views and the platform's own text and
field; a hundred copies of a JavaScript digit tree do not reach a steady
state inside the run, and the 24-copy mount section above is their measure.
...
| --- | --- | --- | --- | --- |
| Text (no animation) | 123.2 KB | 28.1 KB | 40.5 KB | 1 |
| **Nitro `value` prop** | 79.2 KB | 57.1 KB | 81.8 KB | 1 |

#### iPhone 11 Pro (iPhone12,3, iOS 26.6.1, 60 Hz)

| Implementation | footprint per copy | malloc per copy | left behind per copy | runs |
| --- | --- | --- | --- | --- |
| Text (no animation) | 122.2 KB | 28.2 KB | 25.6 KB | 1 |
| **Nitro `value` prop** | 60.5 KB | 56.9 KB | 54.6 KB | 1 |

#### Samsung Galaxy A22 (SM-A225F, Android 13 (API 33), 90 Hz)

| Implementation | footprint per copy | malloc per copy | Java heap per copy | left behind per copy | runs |
| --- | --- | --- | --- | --- | --- |
| Text (no animation) | 80.8 KB | 31.8 KB | 48.5 KB | 41.5 KB | 1 |
| **Nitro `value` prop** | 60.7 KB | 54.3 KB | 8.7 KB | 51.5 KB | 1 |
