...
sidebar_position: 8
title: Benchmarks
description: Typing, focus and mount cost of NitroInput, plain and reflowing, against TextInput, the JS-formatted amount fields, the mask libraries and Expo UI, on three phones.
---
...
Eleven fields typed into at eight keys a second by the probe, the way a
keyboard types. The figure to read first is **rewritten keys**: a field that
formats in JavaScript shows the raw keystroke first and the formatted text a
frame or two later, which is the flicker people report with formatted
inputs; NitroInput, plain and reflowing, format inside the native edit and never
rewrite a key. Then **JS per key**, what a fast typist costs the app's JS
thread, and how long a key takes to **settle**. Focus latency and mount cost
have their own sections below.

<BenchChart group="type|8" metrics={['rewrites.keysWithRewrites', 'jsMsPerKey', 'settledMs.p95']} caption="Hover a bar for every column of its row; the tables below have the same figures." />
