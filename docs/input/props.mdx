---
sidebar_position: 7
title: Props
description: 'Every prop of NitroInput and MorphInput.'
---

The full table is in the package [README](https://github.com/ronickg/react-native-nitro-rolling-number/tree/main/packages/react-native-nitro-input#props). The ones you will reach for first:

| Prop | Default | |
| --- | --- | --- |
| `mode` | `'text'` | `'number'` formats as you type, `'mask'` applies a pattern. |
| `fractionDigits`, `maxIntegerDigits` | `2`, `15` | Limits; digits past them are rejected. |
| `groupingSeparator`, `decimalSeparator` | `','`, `'.'` | |
| `mask`, `maskNotations`, `maskAutocomplete`, `maskAutoSkip` | `''`, `[]`, `true`, `false` | `'mask'`: the pattern, your own slots, and how literals are inserted and deleted — see [A masked field](/input/masked-field). |
| `prefix`, `suffix`, `affixAlign` | `''`, `''`, `'baseline'` | Drawn at `prefixFontSize` / `suffixFontSize`; `prefixAlign` / `suffixAlign` override one side. |
| `signPlacement` | `'beforeAffix'` | Where a negative amount's sign sits relative to `prefix`: `-$1,234.56`, or `$-1,234.56` with `'afterAffix'`. Morph only — see [Negative amounts](/input/amount-field#negative-amounts). |
| `placeholder`, `placeholderTextColor` | `''` | Shown instead of a value. |
| `variant`, `label`, `labelBehavior` | `'none'`, `''`, `'float'` | A frame and a floating label — see [Frames](/input/frames). |
| `strokeColor`, `focusedStrokeColor`, `strokeWidth`, `cornerRadius`, `fillColor` | | The frame's stroke, corners and fill. |
| `multiline`, `numberOfLines`, `textAlignVertical`, `scrollEnabled` | `false`, `0`, `'auto'`, `true` | A wrapping field — see [Multiline](/input/multiline). |
| `morph`, `duration`, `easing`, `bounce`, `effect` | `false`, `400`, `'expo'`, `0.15`, `'auto'` | The opt-in animation — see [The morph](/input/morph). |
| `autoWidth` | `false` | `false` takes its width from the parent like a `TextInput`; `'auto'` sizes to the content (what `MorphInput` uses). |
| `fontSize`, `fontWeight`, `fontFamily`, `color`, `lineHeight` | | Typography, resolved like `Text`. |
| `textAlign` | `'auto'` | The start edge of the layout direction, as `TextInput`; `'left'`, `'center'` and `'right'` are absolute. |
| `adjustsFontSizeToFit`, `minimumFontScale`, `allowFontScaling`, `maxFontSizeMultiplier` | `false`, `0.5`, `false`, `0` | Shrinking to fit a width, and following the system text size. |
| `cursorColor`, `selectionColor`, `caretHidden` | | The caret and the selection. |
| `keyboardType`, `returnKeyType`, `autoCapitalize`, `autoCorrect`, `maxLength`, `editable`, `autoFocus` | | Input traits. |
| `transform` | | A worklet mask, run synchronously on the UI thread. |
| `onChangeText`, `onChangeValue`, `onFocus`, `onBlur`, `onSubmitEditing`, `onEndEditing`, `onSelectionChange`, `onKeyPress` | | `TextInput`'s edit callbacks, plus `onChangeValue`. Any of them marked `'worklet'` runs on the UI thread instead of the JS one — see [Worklets](/input/worklets). |
| `onChangeMask` | | `'mask'`: the formatted text, the extracted value, the missing tail and whether every mandatory slot is filled. |
| `value`, `defaultValue` | | Controlled and uncontrolled text. A `value` with a stale `mostRecentEventCount` is ignored, like `TextInput`. |
| `submitBehavior` | `'blurAndSubmit'` | `'submit'` keeps focus so a form can move to the next field itself. |
| `secureTextEntry` | `false` | Draws bullets; the field keeps the real text for autofill. |
| `textContentType` / `autoComplete` | `''` | Autofill: `'username'`, `'password'`, `'oneTimeCode'`, … |
| `keyboardAppearance`, `enablesReturnKeyAutomatically`, `showSoftInputOnFocus` | | Keyboard behaviour. |
| `selectTextOnFocus`, `clearTextOnFocus`, `contextMenuHidden`, `spellCheck`, `readOnly` | | As on `TextInput`. |
| `selection` | | `{ start, end? }` in code points, controlled. |

Imperative handle: `focus()`, `blur()`, `clear()`, `setText()`, `setValue()`, `getText()`, `getValue()`, `isFocused()`.

Hook: `useNitroInputState(useSharedValue)` — the field's `text`, `value`, `focused` and `selection` as shared values, plus the worklet `handlers` that keep them current. See [the field's state as shared values](/input/worklets#the-fields-state-as-shared-values).
