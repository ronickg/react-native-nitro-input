...
For a fixed pattern, [`mode="mask"`](/input/masked-field) does this natively with no worklets at all. `transform` is for what a pattern cannot express.
