...
| `revealGrow` | `number` | `0.2` | `count`: how much smaller the figure opens (a fraction of its size), growing to full size over the count. |
...
| `textAlign` | `'auto' \| 'left' \| 'center' \| 'right'` | `'auto'` | Alignment inside a wider frame. `'auto'` is the start edge of the layout direction, as `Text`; `'left'` and `'right'` are absolute. In a right-to-left app the prefix sits at the right edge and the suffix at the left, and the digits keep reading left to right; a single figure can be flipped with `style={{ direction: 'rtl' }}`. |
