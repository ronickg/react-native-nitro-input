...
`NitroText` is the reflow on a label, not a field: read-only, never focused, sized to its text. Give it a new string and the characters the two share glide to their new places while the rest fade, as Torph does on the web: "Sign in" → "Signing in…", "Total $1,204" → "Total $1,318".
...
It takes the reflow's timing (`duration`, `easing`, `bounce`, `effect`) and the typography props, and screen readers read the text.
