...
<Prop name="groupingSizes" type="number[]" default="[3]">
...
<Prop name="signDisplay" type="'auto' | 'always' | 'exceptZero' | 'negative' | 'never'" default="'auto'">
...
<Prop name="digits" type="Record<number, { max: number }>">
The highest digit an integer position (0 the ones, 1 the tens…) shows before it wraps to 0, as NumberFlow's `digits`. `{ 1: { max: 5 } }` makes the tens of a clock's seconds wrap after 5, so 59 → 00 turns them one step instead of five back. [`NitroTime`](/docs/nitro-number#clocks-and-timers) sets it for you.
</Prop>

## Roll timing

<Prop name="animated" type="boolean" default="true">
`false` shows every change at once, a reveal included; the flash and the pop still play.
</Prop>

<Prop name="respectReduceMotion" type="boolean" default="true">
Snap while the system's Reduce Motion is on. `false` animates regardless, for a figure whose motion carries meaning.
</Prop>

<Prop name="continuous" type="boolean" default="false">
...
<Prop name="direction" type="'auto' | 'up' | 'down' | 'shortest'" default="'auto'">
...
<Prop name="shimmerDirection" type="'auto' | 'ltr' | 'rtl'" default="'auto'">
Which way the band sweeps; `'auto'` follows the layout direction. `'rtl'` mirrors the slant too.
</Prop>

<Prop name="shimmerDelay" type="number" default="0">
...
<Prop name="prefixColor, suffixColor" type="ColorValue" default="color">
The affixes' colours: a muted currency symbol or unit.
</Prop>

<Prop name="fractionFontSize" type="number" default="fontSize">
The fraction digits' and decimal separator's size: smaller cents.
</Prop>

<Prop name="fractionColor" type="ColorValue" default="color">
The fraction digits' and decimal separator's colour: dimmer cents.
</Prop>

<Prop name="fractionAlign" type="'baseline' | 'center' | 'top' | 'bottom'" default="'baseline'">
How smaller fraction digits line up with the integer ones; `'top'` sets superscript cents ("$12⁹⁹").
</Prop>

<Prop name="tabularNums" type="boolean" default="true">
...
<Prop name="onAnimationStart" type="() => void">
Called when the figure starts moving from rest: a change, or a reveal.
</Prop>

<Prop name="onAnimationEnd" type="(value: number) => void">
