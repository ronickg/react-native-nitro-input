///
/// HybridNitroNumberViewSpec.swift
/// This file was generated by nitrogen. DO NOT MODIFY THIS FILE.
/// https://github.com/mrousavy/nitro
/// Copyright © Marc Rousavy @ Margelo
///

import NitroModules

/// See ``HybridNitroNumberViewSpec``
public protocol HybridNitroNumberViewSpec_protocol: HybridObject, HybridView {
  // Properties
  var value: Double { get set }
  var fractionDigits: Double? { get set }
  var minimumIntegerDigits: Double? { get set }
  var groupingSeparator: String? { get set }
  var decimalSeparator: String? { get set }
  var prefix: String? { get set }
  var suffix: String? { get set }
  var duration: Double? { get set }
  var easing: NitroNumberEasing? { get set }
  var bounce: Double? { get set }
  var stagger: Double? { get set }
  var transition: NitroNumberTransition? { get set }
  var flashUpColor: Double? { get set }
  var flashDownColor: Double? { get set }
  var flashDuration: Double? { get set }
  var popOnChange: Double? { get set }
  var rollDirection: NitroNumberDirection? { get set }
  var revealState: Double? { get set }
  var revealStyle: NitroNumberRevealStyle? { get set }
  var revealDuration: Double? { get set }
  var revealBounce: Double? { get set }
  var revealGrow: Double? { get set }
  var revealStagger: Double? { get set }
  var revealMilestones: [Double]? { get set }
  var revealMilestoneHold: Double? { get set }
  var loading: Bool? { get set }
  var shimmerColor: Double? { get set }
  var shimmerDuration: Double? { get set }
  var fontSize: Double? { get set }
  var prefixFontSize: Double? { get set }
  var suffixFontSize: Double? { get set }
  var affixAlign: NitroNumberAffixAlign? { get set }
  var prefixAlign: NitroNumberAffixAlign? { get set }
  var suffixAlign: NitroNumberAffixAlign? { get set }
  var letterSpacing: Double? { get set }
  var prefixSpacing: Double? { get set }
  var suffixSpacing: Double? { get set }
  var prefixOffset: Double? { get set }
  var suffixOffset: Double? { get set }
  var tabularNums: Bool? { get set }
  var adjustsFontSizeToFit: Bool? { get set }
  var minimumFontScale: Double? { get set }
  var allowFontScaling: Bool? { get set }
  var maxFontSizeMultiplier: Double? { get set }
  var fontWeight: Double? { get set }
  var fontFamily: String? { get set }
  var color: Double? { get set }
  var textAlign: NitroNumberTextAlign? { get set }
  var rightToLeft: Bool? { get set }
  var onSizeChange: ((_ width: Double, _ height: Double) -> Void)? { get set }
  var onRevealEnd: (() -> Void)? { get set }
  var onRevealMilestone: ((_ index: Double, _ value: Double) -> Void)? { get set }

  // Methods
  func jumpTo(value: Double) throws -> Void
  func animateTo(value: Double) throws -> Void
  func revealTo(value: Double) throws -> Void
}

public extension HybridNitroNumberViewSpec_protocol {
  /// Default implementation of ``HybridObject.toString``
  func toString() -> String {
    return "[HybridObject NitroNumberView]"
  }
}

/// See ``HybridNitroNumberViewSpec``
open class HybridNitroNumberViewSpec_base {
  private weak var cxxWrapper: HybridNitroNumberViewSpec_cxx? = nil
  public init() { }
  public func getCxxWrapper() -> HybridNitroNumberViewSpec_cxx {
  #if DEBUG
    guard self is any HybridNitroNumberViewSpec else {
      fatalError("`self` is not a `HybridNitroNumberViewSpec`! Did you accidentally inherit from `HybridNitroNumberViewSpec_base` instead of `HybridNitroNumberViewSpec`?")
    }
  #endif
    if let cxxWrapper = self.cxxWrapper {
      return cxxWrapper
    } else {
      let cxxWrapper = HybridNitroNumberViewSpec_cxx(self as! any HybridNitroNumberViewSpec)
      self.cxxWrapper = cxxWrapper
      return cxxWrapper
    }
  }
}

/**
 * A Swift base-protocol representing the NitroNumberView HybridObject.
 * Implement this protocol to create Swift-based instances of NitroNumberView.
 * ```swift
 * class HybridNitroNumberView : HybridNitroNumberViewSpec {
 *   // ...
 * }
 * ```
 */
public typealias HybridNitroNumberViewSpec = HybridNitroNumberViewSpec_protocol & HybridNitroNumberViewSpec_base
