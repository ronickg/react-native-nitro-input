...
class HybridNitroInputView(private val context: ThemedReactContext) : HybridNitroInputViewSpec(), RecyclableView {
  ...
  override var suffixAlign: NitroInputAffixAlign = NitroInputAffixAlign.BASELINE
    set(v) { field = v; markConfigDirty() }
  override var letterSpacing: Double = 0.0
    set(v) { field = v; markConfigDirty() }
  override var prefixSpacing: Double = Double.POSITIVE_INFINITY
    set(v) { field = v; markConfigDirty() }
  override var suffixSpacing: Double = Double.POSITIVE_INFINITY
    set(v) { field = v; markConfigDirty() }
  override var prefixOffset: Double = 0.0
    set(v) { field = v; markConfigDirty() }
  override var suffixOffset: Double = 0.0
    set(v) { field = v; markConfigDirty() }
  ...
  override fun prepareForRecycle() {
    ...
    letterSpacing = 0.0
    prefixSpacing = Double.POSITIVE_INFINITY
    suffixSpacing = Double.POSITIVE_INFINITY
    prefixOffset = 0.0
    suffixOffset = 0.0
  ...
  private fun flushConfigIfNeeded() {
    ...
    inputView.typography = NitroInputView.Typography(
      ...
      letterSpacing = if (letterSpacing.isFinite()) letterSpacing.toFloat() else 0f,
      prefixSpacing = prefixSpacing.takeIf { it.isFinite() }?.toFloat(),
      suffixSpacing = suffixSpacing.takeIf { it.isFinite() }?.toFloat(),
      prefixOffset = if (prefixOffset.isFinite()) prefixOffset.toFloat() else 0f,
      suffixOffset = if (suffixOffset.isFinite()) suffixOffset.toFloat() else 0f,
