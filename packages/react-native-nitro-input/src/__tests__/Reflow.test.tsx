...
describe('NitroInput with transition="reflow"', () => {
  it('sends the defaults for every native prop', () => {
    const props = nativeProps(render(<NitroInput transition="reflow" />))
    ...
    expect(props.suffixAlign).toBe('baseline')
    expect(props.prefixAlign).toBe('baseline')
    expect(props.color).toBe(Infinity)
    expect(props.placeholderColor).toBe(Infinity)
  ...
  it('maps the React Native aliases onto the native props', () => {
    ...
    expect(nativeProps(render(<NitroInput transition="reflow" blurOnSubmit={false} />)).submitBehavior).toBe('submit')
    expect(nativeProps(render(<NitroInput transition="reflow" submitBehavior="submit" blurOnSubmit />)).submitBehavior).toBe('submit')
    // `readOnly` is an alias of `editable={false}`.
    expect(nativeProps(render(<NitroInput transition="reflow" readOnly />)).editable).toBe(false)
    expect(nativeProps(render(<NitroInput transition="reflow" editable={false} />)).editable).toBe(false)
    // `autoComplete` fills in for `textContentType` when that is not given.
    expect(nativeProps(render(<NitroInput transition="reflow" autoComplete="username" />)).textContentType).toBe('username')
    expect(
      nativeProps(render(<NitroInput transition="reflow" autoComplete="username" textContentType="password" />)).textContentType
    ).toBe('password')
    // `spellCheck` follows `autoCorrect` unless it is set itself.
    expect(nativeProps(render(<NitroInput transition="reflow" autoCorrect={false} />)).spellCheck).toBe(false)
    expect(nativeProps(render(<NitroInput transition="reflow" autoCorrect={false} spellCheck />)).spellCheck).toBe(true)
  })

  it('renders the plain field with the overlay off and the reflowing one with it on', () => {
    expect(nativeProps(render(<NitroInput transition="reflow" />)).plain).toBe(false)
  ...
  it('forwards the field identity so the hidden system field carries it', () => {
    const props = nativeProps(render(<NitroInput transition="reflow" testID="amount" accessibilityLabel="Amount" />))
    expect(props.fieldTestID).toBe('amount')
  ...
  it('sends a controlled selection and nothing when there is none', () => {
    const none = nativeProps(render(<NitroInput transition="reflow" />))
    expect(none.selectionStart).toBe(-1)
    expect(none.selectionEnd).toBe(-1)
    const caret = nativeProps(render(<NitroInput transition="reflow" selection={{ start: 3 }} />))
    expect(caret.selectionStart).toBe(3)
    expect(caret.selectionEnd).toBe(3)
    const range = nativeProps(render(<NitroInput transition="reflow" selection={{ start: 1, end: 4 }} />))
    expect(range.selectionStart).toBe(1)
    expect(range.selectionEnd).toBe(4)
  })

  it('never sends undefined for a native prop', () => {
    const props = nativeProps(render(<NitroInput transition="reflow" mode="number" value="1,234" />))
  ...
  it('picks a numeric keyboard in number mode', () => {
    expect(nativeProps(render(<NitroInput transition="reflow" mode="number" />)).keyboardType).toBe('decimal-pad')
    expect(nativeProps(render(<NitroInput transition="reflow" mode="number" fractionDigits={0} />)).keyboardType).toBe('number-pad')
    expect(nativeProps(render(<NitroInput transition="reflow" mode="number" keyboardType="numeric" />)).keyboardType).toBe('numeric')
  })

  it('converts colors and font weights', () => {
    const props = nativeProps(render(<NitroInput transition="reflow" color="#ff0000" fontWeight="bold" placeholderTextColor="#00ff00" />))
  ...
  it('keeps an uncontrolled field on its initial text', () => {
    const renderer = render(<NitroInput transition="reflow" defaultValue="hello" />)
    expect(nativeProps(renderer).text).toBe('hello')
    act(() => {
      renderer.update(<NitroInput transition="reflow" defaultValue="changed" />)
    })
    expect(nativeProps(renderer).text).toBe('hello')
  })
  ...
  it('maps onFocusChange to onFocus and onBlur', () => {
    ...
    const renderer = render(<NitroInput transition="reflow" onFocus={onFocus} onBlur={onBlur} />)
  ...
  it('reports the event count of the latest native edit back to native', () => {
    ...
    const renderer = render(<NitroInput transition="reflow" value="1" onChangeText={onChangeText} />)
  ...
  it('keeps the callback props stable across renders', () => {
    const renderer = render(<NitroInput transition="reflow" onChangeText={() => {}} onFocus={() => {}} />)
    const first = nativeProps(renderer)
    act(() => {
      renderer.update(<NitroInput transition="reflow" onChangeText={() => {}} onFocus={() => {}} />)
    })
  ...
  it('sizes itself from onSizeChange unless the style says otherwise', () => {
    const renderer = render(<NitroInput transition="reflow" fontSize={40} />)
  ...
  it('sends no worklet ids when worklets are unavailable', () => {
    ...
    const props = nativeProps(render(<NitroInput transition="reflow" transform={transform} onChangeText={onChangeText} />))
  ...
  it('does not call a worklet handler on the JS thread as well', () => {
    ...
    const renderer = render(<NitroInput transition="reflow" onSubmitEditing={onSubmitEditing} onKeyPress={onKeyPress} />)
    const props = nativeProps(renderer)
    act(() => {
  ...
  it('still calls a plain handler on the JS thread', () => {
    ...
    const renderer = render(<NitroInput transition="reflow" onSubmitEditing={onSubmitEditing} />)
    const props = nativeProps(renderer)
    act(() => {
      ;(props.onSubmitEditing as { f: (t: string) => void }).f('done')
    })
  ...
  it('exposes a handle whose methods are no-ops before mount', () => {
    ...
    render(<NitroInput transition="reflow" ref={ref} defaultValue="abc" />)
  ...
  it('passes the mask through only in mask mode', () => {
    ...
    const text = nativeProps(render(<NitroInput transition="reflow" mask="[000]" />))
    expect(text.mode).toBe('text')
    expect(text.mask).toBe('')
  })

  it('keeps the mask callback stable across renders', () => {
    const renderer = render(<NitroInput transition="reflow" mode="mask" mask="[000]" onChangeMask={() => {}} />)
    const first = nativeProps(renderer).onChangeMask
    act(() => {
      renderer.update(<NitroInput transition="reflow" mode="mask" mask="[000]" onChangeMask={() => {}} />)
    })
    expect(nativeProps(renderer).onChangeMask).toBe(first)
  })
  it('maps inputMode and enterKeyHint like React Native does', () => {
    expect(nativeProps(render(<NitroInput transition="reflow" inputMode="decimal" />)).keyboardType).toBe('decimal-pad')
    expect(nativeProps(render(<NitroInput transition="reflow" inputMode="tel" />)).keyboardType).toBe('phone-pad')
    expect(nativeProps(render(<NitroInput transition="reflow" inputMode="email" />)).keyboardType).toBe('email-address')
    expect(nativeProps(render(<NitroInput transition="reflow" enterKeyHint="go" />)).returnKeyType).toBe('go')
    expect(nativeProps(render(<NitroInput transition="reflow" enterKeyHint="enter" />)).returnKeyType).toBe('default')

    // `inputMode="none"` focuses without a keyboard.
    const none = nativeProps(render(<NitroInput transition="reflow" inputMode="none" />))
    ...
    const explicit = nativeProps(
      render(<NitroInput transition="reflow" inputMode="tel" keyboardType="url" enterKeyHint="go" returnKeyType="send" />)
    )
  ...
  it('fires onChange alongside onChangeText', () => {
    ...
    const onChangeText = jest.fn()
    const renderer = render(<NitroInput transition="reflow" onChange={onChange} onChangeText={onChangeText} />)
