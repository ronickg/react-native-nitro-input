/**
 * The docs' props reference (`docs/content/nitro-number-props.mdx`) and the README's
 * `NitroNumber props` table are derived from `NitroNumberProps` and
 * `NitroNumberHandle`, not
...
const source = readFileSync(join(__dirname, '..', 'NitroNumber.tsx'), 'utf8')
const props = membersOfInterface(source, 'NitroNumberProps')
const methods = membersOfInterface(source, 'NitroNumberHandle')
const fromViewProps = new Set(['style', 'testID', 'accessibilityLabel'])

/** The README documents both components; only its `NitroNumber props` section is this one's. */
...
describe.each([
  ['docs page', () => readFileSync(join(__dirname, '..', '..', '..', '..', 'docs', 'content', 'nitro-number-props.mdx'), 'utf8')],
  ['README', readmeSection],
])('%s NitroNumber props reference', (_name, read) => {
...
describe('docs page methods', () => {
  const documented = documentedIn(readFileSync(join(__dirname, '..', '..', '..', '..', 'docs', 'content', 'nitro-number-props.mdx'), 'utf8'))
